...
plt.figure(figsize=(12, 10))  # 增大图像大小
sns.heatmap(df.isnull(), cbar=False, cmap='viridis')  # 使用 cmap='viridis' 可以使缺失值更易于识别
plt.subplots_adjust(left=0.2, right=0.8, top=0.7, bottom=0.3)
plt.xticks(rotation=90, ha='right', fontsize=8)  # 旋转特征名称并调整字体大小

plt.yticks(rotation=0, fontsize=8)   # y 轴的特征名称不需要旋转，保持水平显示，并减小字体大小
plt.savefig('MissValueBefore.pdf')
plt.show()
...
with sns.axes_style('white'):
    ...
    plt.xticks(rotation=45, ha='right', fontsize=10)  # 旋转特征名称并调整字体大小
    plt.yticks(rotation=0, fontsize=10)  # 调整字体大小
    # plt.savefig('CorrelationMaterixAfter.pdf')
    plt.show()
...
# 将处理好的数据保存为新的 CSV 文件
processed_data = pd.concat([pd.DataFrame(y_tr_resample), pd.DataFrame(x_tr_resample, columns=X_train.columns)], axis=1)
processed_data.to_csv('processed_transaction_data.csv', index=False)
